    ...
    <main className="mx-auto max-w-7xl space-y-10 px-4 py-8 sm:px-6 lg:px-8 lg:py-10">
      <section>
        ...
        <div className="mt-2 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div><h1 className="text-3xl font-semibold tracking-[-0.04em] sm:text-4xl">Make the next cut.</h1><p className="mt-2 text-sm text-muted-foreground">Upload long footage and turn it into a polished, platform-ready video.</p></div>
      ...
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {metrics.map((metric) => (
          <Card key={metric.label} className="border-white/[0.07] bg-card/55">
      ...
      <section className="grid gap-5 lg:grid-cols-[minmax(0,1.5fr)_minmax(260px,0.5fr)]">
        <Card className="border-white/[0.08] bg-card/55">
          <CardContent className="p-4 sm:p-6"><div className="mb-5"><h2 className="font-medium">New video project</h2><p className="mt-1 text-sm text-muted-foreground">Your upload starts processing as soon as it completes.</p></div><UploadZone /></CardContent>
        </Card>
        <Card className="border-white/[0.08] bg-card/55">
      ...
      <section><div className="mb-5 flex items-end justify-between"><div><h2 className="text-xl font-semibold tracking-[-0.025em]">Recent projects</h2><p className="mt-1 text-sm text-muted-foreground">Continue editing or download completed exports.</p></div><span className="text-xs text-muted-foreground">{projects.length} total</span></div><ProjectHistory projects={projects} /></section>
    </main>
  );
}
