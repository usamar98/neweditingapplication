...
import { Camera, CircleAlert, CreditCard, LoaderCircle, PauseCircle, Save, Trash2 } from "lucide-react";
...
export function AccountSettings(props: AccountProps) {
  ...
  return (
    <main className="mx-auto max-w-5xl space-y-6 px-4 py-8 sm:px-6 lg:px-8 lg:py-10">
      ...
      <Card className="border-white/[0.08] bg-card/55"><CardHeader><CardTitle>Profile</CardTitle></CardHeader><CardContent className="space-y-6">
      ...
      <Card id="billing" className="scroll-mt-24 border-white/[0.08] bg-card/55"><CardHeader><CardTitle className="flex items-center gap-2"><CreditCard className="size-5 text-primary" /> Billing & subscription</CardTitle></CardHeader><CardContent><div className="flex flex-col gap-5 rounded-xl border border-white/[0.07] bg-black/10 p-5 sm:flex-row sm:items-center sm:justify-between"><div><div className="flex items-center gap-2"><p className="font-medium capitalize">{props.billing.plan ?? "No paid plan"}</p>{props.billing.status && <Badge variant="outline" className="capitalize">{props.billing.status.replaceAll("_", " ")}</Badge>}</div><p className="mt-2 text-sm text-muted-foreground">{props.billing.cancelAtPeriodEnd ? "Cancellation is scheduled at the end of the billing period." : props.billing.subscriptionId ? "Stripe securely manages invoices, payment methods, plan changes, and cancellation." : "Choose a plan from Pricing to start a subscription."}</p></div><div className="flex flex-wrap gap-2">{props.billing.subscriptionId ? <><Button variant="outline" onClick={() => void openPortal("manage")}>Manage billing</Button><Button variant="destructive" onClick={() => void openPortal("cancel")}>Cancel subscription</Button></> : <Button asChild><a href="/#pricing">View plans</a></Button>}</div></div></CardContent></Card>

      <Card className="border-destructive/20 bg-destructive/[0.035]"><CardHeader><CardTitle>Account lifecycle</CardTitle></CardHeader><CardContent className="grid gap-4 sm:grid-cols-2"><div className="rounded-xl border border-white/[0.07] p-5"><PauseCircle className="size-5 text-amber-300" /><h3 className="mt-4 font-medium">Deactivate account</h3><p className="mt-2 text-sm leading-6 text-muted-foreground">Temporarily blocks workspace access. You can sign in and reactivate later. Active subscriptions must be cancelled first.</p><Dialog><DialogTrigger asChild><Button variant="outline" className="mt-5">Deactivate</Button></DialogTrigger><DialogContent><DialogHeader><DialogTitle>Deactivate your account?</DialogTitle><DialogDescription>Your projects remain stored, but workspace access stops until you reactivate.</DialogDescription></DialogHeader><DialogFooter><Button variant="destructive" onClick={() => startTransition(async () => { const result = await deactivateAccount(); if (result?.error) setMessage(result.error); })}>Deactivate account</Button></DialogFooter></DialogContent></Dialog></div><div className="rounded-xl border border-destructive/20 p-5"><Trash2 className="size-5 text-red-300" /><h3 className="mt-4 font-medium">Delete account permanently</h3><p className="mt-2 text-sm leading-6 text-muted-foreground">Cancels an active subscription, removes private files and projects, and deletes your login. This cannot be undone.</p><Dialog><DialogTrigger asChild><Button variant="destructive" className="mt-5">Delete account</Button></DialogTrigger><DialogContent><DialogHeader><DialogTitle>Permanently delete this account?</DialogTitle><DialogDescription>Enter your current password to confirm. All stored media and account data will be removed.</DialogDescription></DialogHeader><div className="space-y-2"><Label htmlFor="delete-password">Current password</Label><Input id="delete-password" type="password" value={password} onChange={(event) => setPassword(event.target.value)} autoComplete="current-password" /></div><DialogFooter><Button variant="destructive" disabled={!password || pending} onClick={() => startTransition(async () => { const result = await deleteAccount(password); if (result?.error) setMessage(result.error); })}>{pending && <LoaderCircle className="size-4 animate-spin" />}Delete forever</Button></DialogFooter></DialogContent></Dialog></div></CardContent></Card>
    </main>
  );
}
