...
export function AccountSettings(props: AccountProps) {
  ...
  const creditUsagePercent = props.credits.allocatedCredits > 0
    ...
    : 0;
  const showsWelcomeCredits = props.credits.accessMode === "welcome" || (!props.credits.active && props.credits.welcomeClaimed);
  ...
  return (
    <main className="mx-auto max-w-5xl space-y-6 px-4 py-8 sm:px-6 lg:px-8 lg:py-10">
      ...
      <Card id="billing" className="scroll-mt-24 border-border bg-card/70"><CardHeader><CardTitle className="flex items-center gap-2"><CreditCard className="size-5 text-primary" /> Billing & subscription</CardTitle></CardHeader><CardContent className="space-y-4"><div className="flex flex-col gap-5 rounded-xl border border-border bg-muted/55 p-5 sm:flex-row sm:items-center sm:justify-between"><div><div className="flex items-center gap-2"><p className="font-medium capitalize">{props.billing.plan ?? "No paid plan"}</p>{props.billing.status ? <Badge variant="outline" className="capitalize">{props.billing.status.replaceAll("_", " ")}</Badge> : null}</div><p className="mt-2 text-sm text-muted-foreground">{props.billing.cancelAtPeriodEnd ? "Cancellation is scheduled at the end of the billing period." : props.billing.subscriptionId ? "Stripe securely manages invoices, payment methods, plan changes, and cancellation." : "Choose a plan from Pricing to start a subscription."}</p><p className="mt-3 text-xs text-muted-foreground"><Link href="/legal/subscriptions-credits-refunds" className="font-medium text-foreground underline underline-offset-4">Subscription, credits, cancellation & refunds</Link></p></div><div className="flex flex-wrap gap-2">{props.billing.subscriptionId ? <><Button variant="outline" onClick={() => void openPortal("manage")}>Manage billing</Button><Button variant="destructive" onClick={() => void openPortal("cancel")}>Cancel subscription</Button></> : <Button asChild><a href="/#pricing">View plans</a></Button>}</div></div><div className="rounded-xl border border-border bg-card/60 p-5"><div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div className="flex items-start gap-3"><span className="grid size-9 place-items-center rounded-lg bg-primary/10 text-primary"><Coins className="size-4" /></span><div><p className="font-medium">{showsWelcomeCredits ? "One-time welcome credits" : "Monthly credits"}</p><p className="mt-1 text-xs text-muted-foreground">{showsWelcomeCredits ? "Usable for four images with the fixed welcome model; a paid plan unlocks model selection." : "Reserved credits are held while work is queued or processing."}</p></div></div><div className="text-left sm:text-right"><p className="text-2xl font-semibold tracking-[-0.03em]">{props.credits.remainingCredits.toLocaleString()}</p><p className="text-xs text-muted-foreground">of {props.credits.allocatedCredits.toLocaleString()} remaining</p></div></div><Progress value={creditUsagePercent} className="mt-5 h-2" /><div className="mt-3 flex flex-wrap gap-x-5 gap-y-2 text-xs text-muted-foreground"><span>{props.credits.consumedCredits.toLocaleString()} used</span><span>{props.credits.reservedCredits.toLocaleString()} reserved</span><span>{props.credits.activeGenerations}/{props.credits.concurrencyLimit} active slots</span>{!showsWelcomeCredits && props.credits.periodEnd ? <span>Resets {new Intl.DateTimeFormat("en", { dateStyle: "medium" }).format(new Date(props.credits.periodEnd))}</span> : null}</div></div></CardContent></Card>
